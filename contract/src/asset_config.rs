...
impl AssetConfig {
    ...
    pub fn get_rate(
        ...
        if total_supplied_balance == 0 {
            ...
            // total_supplied_balance  = self.supplied.balance
            ...
            log!("get_rate: borrowed_balance =>  {}", borrowed_balance);
            log!("get_rate: total_supplied_balance =>  {}", total_supplied_balance);
            log!("get_rate: current_utilization => {}", current_utilization);
            log!("get_rate: target_utilization => {}", target_utilization);
            log!("get_rate: target_utilization_rate => {}", BigDecimal::from(self.target_utilization_rate));

            // if current_utilization <= target_utilization,
            //     r = target_utilization_rate * current_utilization
            // if current_utilization > target_utilization,
            //     r = target_utilization_rate * current_utilization + (current_utilization - target_utilization) * max_utilization_rate

            if current_utilization <= target_utilization {
                // = target_utilization_rate * current_utilization
                // BigDecimal::from(self.target_utilization_rate) * current_utilization

                // Rt = 0 + (0.058064516129032258064516129 * (1.000000001243680656318820313 - 1) / 0.8) =
                // Rt = R0 + (Ut/Uoptimal) * Rslope1
                BigDecimal::one()               // R0
                    + current_utilization       // Ut
                        * (BigDecimal::from(self.target_utilization_rate) - BigDecimal::one()) // R slope1
                        / target_utilization //U optimal
            } else {
                // = target_utilization_rate * current_utilization + (current_utilization - target_utilization) * max_utilization_rate
                // Rt = R0 + Rslope1 + ((Ut-Uoptimal)/(1 - Uoptimal)) * Rslope2
                BigDecimal::one()               // R0
                    + (BigDecimal::from(self.target_utilization_rate) - BigDecimal::one())      // R slope1
                    + (current_utilization - target_utilization)                                // (Ut - U optimal)
                        * (BigDecimal::from(self.max_utilization_rate) - BigDecimal::one())     // R slope2
                        / (BigDecimal::one() - target_utilization) // 1 - U optimal
            }
        }
    }
}
...
mod tests {
    ...
    fn test_get_rate_and_apr() {
        ...
        let apr = rate.pow(SECOND_PER_YEAR) - BigDecimal::one();
        println!("APR: {}", apr)
    }
}
