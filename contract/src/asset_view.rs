...
pub struct AssetDetailedView {
    ...
    /// Current APY excluding farms for supplying the asset.
    pub supply_apy: BigDecimal,
    /// Current APY excluding farms for borrowing the asset.
...
impl Contract {
    pub fn asset_into_detailed_view(&self, token_id: TokenId, asset: Asset) -> AssetDetailedView {
        let farms = self
            ...
            .collect();
        let supply_apy = asset.get_supply_apy();
        let borrow_apy = asset.get_borrow_apy();
        let Asset {
            supplied,
            borrowed,
            reserved,
